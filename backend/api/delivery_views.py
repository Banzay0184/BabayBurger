    ...
    def handle_callback_query(self, callback_query):
        ...
        try:
            ...
            # Обрабатываем callback для обновления статуса заказа
                ...
                # Автоматически обновляем сообщение с маршрутом
                try:
                    from api.models import User, DeliveryDriver, DeliveryAssignment
                    ...
                    if assignments:
                        ...
                        # Создаем кнопки для процесса доставки
                        keyboard = []
                        for assignment in assignments:
                            order = assignment.order
                            restaurant = order.restaurant
                    ...
                    logger.error(f"Error updating route message: {str(e)}")
                ...
                try:
                    ...
                    if assignments:
                        ...
                        # Если нет активных заказов, показываем соответствующее сообщение
                        route_text = "🗺️ У вас нет активных заказов для маршрута.\n\nПринимайте заказы через группу доставщиков!"
                        self.send_delivery_message(
                            user_id, 
                            route_text,
                    ...
                    logger.error(f"Error updating route message after delivery: {str(e)}")
