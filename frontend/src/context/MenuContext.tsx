...
export const MenuProvider: React.FC<MenuProviderProps> = ({ children }) => {
  ...
  const getCategoriesWithItems = (): MenuCategory[] => {
    return (state.categories || []).map(category => ({
      ...category,
      items: (state.items || []).filter(item => item.category === category.id) // Временно убираем && item.is_active
    }));
  };

  const getAvailableCategories = (): MenuCategory[] => {
    const categories = (state.categories || []).filter(category => 
      (state.items || []).some(item => item.category === category.id) // Временно убираем && item.is_active
    );
  ...
  const getHits = (): MenuItem[] => {
    const hits = (state.items || []).filter(item => item.is_hit).sort((a, b) => a.priority - b.priority); // Временно убираем && item.is_active
  ...
  const getNewItems = (): MenuItem[] => {
    const newItems = (state.items || []).filter(item => item.is_new).sort((a, b) => a.priority - b.priority); // Временно убираем && item.is_active
