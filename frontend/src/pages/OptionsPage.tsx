...
export const OptionsPage: React.FC<OptionsPageProps> = ({ item, onClose }) => {
  ...
  const shouldUseFourColumns = availableAddOns.length > 18;
  
  // Если дополнений очень много, показываем их в 5 колонок
  const shouldUseFiveColumns = availableAddOns.length > 30;
  
  // Если дополнений очень много, показываем их в 6 колонок
  const shouldUseSixColumns = availableAddOns.length > 42;
  
  // Если дополнений очень много, показываем их в 7 колонок
  const shouldUseSevenColumns = availableAddOns.length > 56;
  
  // Если дополнений очень много, показываем их в 8 колонок
  const shouldUseEightColumns = availableAddOns.length > 72;
  
  // Если дополнений очень много, показываем их в 9 колонок
  const shouldUseNineColumns = availableAddOns.length > 90;
  
  // Если дополнений очень много, показываем их в 10 колонок
  const shouldUseTenColumns = availableAddOns.length > 110;
  
  // Если дополнений очень много, показываем их в 11 колонок
  const shouldUseElevenColumns = availableAddOns.length > 132;
  
  // Если дополнений очень много, показываем их в 12 колонок
  const shouldUseTwelveColumns = availableAddOns.length > 156;
